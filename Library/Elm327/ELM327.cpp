...
#include "ELM327.h"
#define DEBUG 1
#define NEW_GET_BYTES 1

#define LOG(...) do { \
if (DEBUG) { \
    char debugCmd[256]; sprintf(debugCmd, __VA_ARGS__); Serial.print(debugCmd); \
} \
} while (0)

static byte getValue(char* input)         
{                                 
    char *output = input;         
    char* startData = strstr(input, "41");
    if (startData == NULL)
    {
        LOG("NO 41\n");
        return ELM_GARBAGE;
    }
    int start = strlen(output) - strlen(startData);
    char hex[] = "0x00";          

    for (int i = start, j = 0; i<strlen(input); i++,j++)
    {                             
    ...
    sprintf(hex, "0x%c%c", output[4], output[5]);
    byte result = strtol(hex, NULL, 16);
    return result; 
}            

Elm327::Elm327(int rx, int tx, unsigned long baudrate)
{
    mySerial = new SoftwareSerial(rx, tx);
    ELM_PORT->begin(baudrate);
}

byte Elm327::begin(){
    LOG("begin()\n");
    char data[64];
    runCommand("ATZ", data,64);
...
byte Elm327::getBytes( const char *mode, const char *chkMode, const char *pid, byte *values, unsigned int numValues){
    ...
    sprintf(hexVal, "0x%c%c", data[6], data[7]);
    values[0] = strtol(hexVal,NULL,16);
    return ELM_SUCCESS;
}
